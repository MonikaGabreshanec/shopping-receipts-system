...
export default function ReceiptProductsForm({ receiptId, initialProducts, onSave }) {
    console.log("receiptId:", receiptId); // should print a number
    ...
    try {
      const payload = {
        products: products.map(p => ({
          ...
          product: p.name, // rename 'name' -> 'product'
          price: parseFloat(p.price)
        }))
      };

      if (onSave) onSave(products); // update parent state first
      alert("Products saved successfully!");
    } catch (err) {
      console.error("Failed to save products:", err.response?.data || err);
